...
{
    std::cout << "С конвейера сошел очередной железяка Claptrap. Назовем его " << name << "." << std::endl;
    return ;
}
...
int				ClapTrap::takeDamage(unsigned int amount)
{
	...
	if (this->_hp < int(amount))
		amount = this->_hp;
	std::cout << this->_name << this->_whod << "получил " << amount << this->_dmg << std::endl;
	this->_hp -= amount;
	if (this->_hp == 0)
	{
...
void			ClapTrap::beRepaired(unsigned int amount)
{
	...
	std::cout << this->_who << this->_name << this->_rpr << amount << " пойнтов жизни" << " и ";
	this->_hp += amount;
	if ((this->_mep - this->_ep) < int(amount))
		amount = this->_mep - this->_ep;
	std::cout << amount << " пойнтов энергии" << std::endl;
	this->_ep += amount;
    return ;
}
